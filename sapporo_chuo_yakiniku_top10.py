...
class SapporoChuoYakinikuTop10Finder:
    def __init__(self):
    ...
    def get_top10_by_reviews(self):
        ...
        return sorted_shops[:10]
    
    def display_top10(self):
        """トップ10を見やすく表示"""
        top10 = self.get_top10_by_reviews()
        ...
        for i, shop in enumerate(top10, 1):
            ...
            # 営業時間（最初の3行のみ表示）
            opening_lines = shop['opening_hours'].split('\n')[:3]
            if opening_lines[0] != '営業時間不明':
                print(f"   🕒 営業時間:")
...
def main():
    ...
    try:
        # 検索システム初期化
        finder = SapporoChuoYakinikuTop10Finder()
